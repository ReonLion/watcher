            ...
            <v-form ref="form">
              <v-layout row wrap>
                ...
                <v-flex md5 xs5 pl-3 pr-3>
                  <verify-code-button :username="username" :email="email" register></verify-code-button>
                </v-flex>
                <v-flex md12 xs12 text-xs-center>
                  <v-btn @click="bindEmail" :loading="loading" color="success" large depressed>绑定邮箱</v-btn>
...
import {patchBindEmail,} from '@/api/api'
    ...
    methods: {
      bindEmail: function() {
        if (this.$refs.form.validate()) {
          ...
          patchBindEmail(this.username, this.email, this.code)
          .then(
            () => {
              this.errMsg = null
              // 刷新用户信息
